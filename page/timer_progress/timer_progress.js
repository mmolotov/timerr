...
Page({
  ...
  buildUI() {
    ...
    this.bars = createTimerPrgressCircle()
  ...
  startTimerProcess() {
    const process = setInterval(() => {
      const reminingSeconds = this.getRemainingSeconds()
      this.setDysplayValue(reminingSeconds)
      this.colorBars(reminingSeconds)
      if (reminingSeconds == 0) {
        this.stopProcesses()
        this.startTimerColorChangerInterval()
        this.createStopButton()
        this.deleteAdditinalButtons()
        this.doVibro()
      }
      else {
        this.notifyOnInterval(reminingSeconds)
      }
    }, INTERVAL_MS)
    this.processes.intervals.push(process)
    this.timerProcess = process
  },

  startTimerColorChangerInterval() {
    let switcher = true
    let interval = setInterval(() => {
      this.timerDisplay.setProperty(prop.MORE, {
        color: switcher ? COLORS.TEXT.TITLE : COLORS.TEXT.WARNING
      })
      switcher = !switcher
    }, 200);
  ...
  notifyOnInterval(reminingSeconds) {
  ...
  setDysplayValue(seconds) {
    this.timerDisplay.setProperty(prop.MORE, {
      text: getTimerDisplayValue(seconds)
    })
  },
  getDysplayValue() {
    return this.timerDisplay.getProperty(prop.TEXT)
  },

  colorBars(reminingSeconds) {
    const progress = Math.min(100, 100 - Math.floor(100 * reminingSeconds / this.timer.valueSeconds))
  ...
  createStopButton() {
    return createWidget(widget.BUTTON, COMMON.STANDARD_BOTTOM_BUTTON_STYLE_ACTION(CONSTANTS.img.STOP, CONSTANTS.img.STOP_PRESSED, (arg) => {
  ...
  createPauseButton() {
    return createWidget(widget.BUTTON, TIMER_PROGRESS.PAUSE_RESUME_BUTTON(CONSTANTS.img.PAUSE, CONSTANTS.img.PAUSE_PRESSED, (button) => {
      this.doVibro()
      this.stopProcesses()
  ...
  createResumeButton() {
    return createWidget(widget.BUTTON, TIMER_PROGRESS.PAUSE_RESUME_BUTTON(CONSTANTS.img.RESUME, CONSTANTS.img.RESUME_PRESSED, (button) => {
      this.doVibro()
  ...
  getRemainingSeconds() {
    const result = Math.floor(Math.max(0, this.end - this.time.getTime()) / CONSTANTS.seconds.mili)
    return result
  },
})

function createTimerPrgressCircle() {
  let bars = {}
  let angle = 360
